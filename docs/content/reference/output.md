...
describes what users get. Pick a format with `-o`, or let deezer choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
deezer <command> -o table   # aligned columns for reading
deezer <command> -o jsonl   # one JSON object per line, for piping
deezer <command> -o json    # a single JSON array
deezer <command> -o csv     # spreadsheet friendly
deezer <command> -o tsv     # tab-separated
deezer <command> -o url     # just the URL column
deezer <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
deezer <command> --fields id,title,url
```
...
```bash
deezer <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
deezer <command>            # a table, because this is a terminal
deezer <command> | wc -l    # JSONL, because this is a pipe
```
